...
function getMaxDistance(word) {
    if (word.length <= 3)
        return 0; // Strict exact match for tiny words unless in specific fixes
    if (word.length <= 5)
        return 1; // 1 typo allowed for small words
...
export function normalizeQuery(query) {
    ...
    const correctedWords = words.map(word => {
        ...
        if (SPECIFIC_FIXES[word])
            return SPECIFIC_FIXES[word];
