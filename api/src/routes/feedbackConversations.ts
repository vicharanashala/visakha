...
function buildFeedbackPipeline(match: object, skip: number, limit: number) {
  return [
    { $match: match },
...
feedbackConversationsRouter.get("/feedback-conversations", async (req: Request, res: Response) => {
  try {
...
feedbackConversationsRouter.get("/feedback-conversations/:conversationId", async (req: Request, res: Response) => {
  try {
    const db = await connectDB();
...
feedbackConversationsRouter.patch("/feedback-conversations/:conversationId/resolved", async (req: Request, res: Response) => {
  try {
    const db = await connectDB();
    const { conversationId } = req.params;
...
feedbackConversationsRouter.get("/", async (req: Request, res: Response) => {
  try {
    const db = await connectDB();
    const page = parseInt(req.query["page"] as string) || 1;
...
function generateViewerHTML(conversations: any[], page: number, totalPages: number, total: number, limit: number): string {
  ...
  const renderContent = (content: any, msgId: string): string => {
    if (typeof content === "string") return `<div class="message-content">${escapeHtml(content)}</div>`;
  ...
  if (conversations.length === 0) {
    ...
    conversations.forEach((conv, convIndex) => {
      ...
      conv.messages.forEach((msg: any, msgIndex: number) => {
        ...
        let contentHTML = "";
        if (isUser && msg.text) contentHTML = `<div class="message-content">${escapeHtml(msg.text)}</div>`;
        else if (!isUser && msg.content) contentHTML = renderContent(msg.content, msgId);
  ...
  if (startPage > 1) { pageNumbers += `<a href="/?page=1&limit=${limit}"><button>1</button></a>`; if (startPage > 2) pageNumbers += `<span class="ellipsis">...</span>`; }
  for (let i = startPage; i <= endPage; i++) pageNumbers += `<a href="/?page=${i}&limit=${limit}"><button class="${i === page ? "active" : ""}">${i}</button></a>`;
  if (endPage < totalPages) { if (endPage < totalPages - 1) pageNumbers += `<span class="ellipsis">...</span>`; pageNumbers += `<a href="/?page=${totalPages}&limit=${limit}"><button>${totalPages}</button></a>`; }
